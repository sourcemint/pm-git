            ...
            return self.callGit([
                ...
                return self.remotes().then(function(remotes) {
                    ...
                    if (
                        remotes &&
                        remotes["origin"] &&
                        remotes["origin"].branches[info.branch] &&
                        remotes["origin"].branches[info.branch].tracking
                    ) {
                        info.tracking = remotes["origin"].branches[info.branch].remote;
                    }
                ...
                if (! (remote = UTIL.trim(remote))) return;
                    ...
                    return self.callGit([
                        ...
                        result.split("\n").forEach(function(line) {
                            if (/Local branches configured for 'git pull':/.test(line)) {
                                section = "git-pull";
                            } else
