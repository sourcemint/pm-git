...
exports.clone = function(pm, options) {
    ...
    if (!PATH.existsSync(cachePath)) {
            ...
            if (options.cached) {
                ...
                if (options.verbose) TERM.stdout.writenl("\0yellow(SKIP: Fetching '" + uri + "' to '" + cachePath + "' (due to --cached).\0)");
            } else {
                return fetch();
            }
        });
    ...
    return Q.when(done, function() {
        ...
        return gitStatus().then(function(gitStatus) {
            ...
            if (status === 200 || !PATH.existsSync(packagePath) || options.delete === true) {
                ...
                if (options.verbose) TERM.stdout.writenl("\0cyan(Copying '" + cachePath + "' to '" + packagePath + "'.\0)");
                
                return FS_RECURSIVE.osCopyDirRecursive(cachePath, packagePath).then(function() {

                    if (options.verbose) TERM.stdout.writenl("\0cyan(Checking out '" + parsedUri.vendor.rev + "' at '" + packagePath + "'.\0)");
                    ...
                    return git.checkout(parsedUri.vendor.rev).then(function() {
                        ...
                        if (parsedUri.vendor.rev.length !== 40) {
                            ...
                            return git.tags().then(function(tags) {
                                ...
                                if (options.verbose) TERM.stdout.writenl("\0cyan(Pulling latest changes at '" + packagePath + "'.\0)");
                        ...
                        if (/pathspec '[^']*' did not match any file\(s\) known to git/.test(err.message)) {
                            ...
                            if (options.verbose) TERM.stdout.writenl("\0cyan(Exact revision '" + parsedUri.vendor.rev + "' not found. Resolving as a revision selector.\0)");
                            ...
                            return git.tags().then(function(tags) {
                                ...
                                if (options.verbose) TERM.stdout.writenl("\0cyan(Checking out '" + tag + "' at '" + packagePath + "'.\0)");
