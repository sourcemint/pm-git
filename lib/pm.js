                    ...
                    return fetchGitStatus(cachePath).then(function(gitStatus) {
                        ...
                        return Q.when(done, function() {
                            ...
                            return git.checkout(ref, {
                                ...
                                // TODO: Pull changes if `isBranch`? We may not need to as latest is already be fetched earlier due to `options.now === true`?
...
exports.edit = function(pm, options) {
    ...
    if (node.status.status.vcs && node.status.status.vcs.mode === "write") {
    ...
    if (!node.status.status.vcs) {
            ...
            opts.vcsOnly = true;
            opts.now = true;
            opts.write = (parsedUri.originalLocatorPM === "git-write" || (parsedRepoUri && parsedRepoUri.originalLocatorPM === "git-write"))?true:false;

            // Try and clone exact version/ref.
            var locator = parsedUri.locators["git-" + ((opts.write)?"write":"read")];
            var version = null;
            ...
            return pm.clone(opts).fail(function(err) {
                TERM.stdout.writenl("\0yellow(" + err.stack + "\0)");
